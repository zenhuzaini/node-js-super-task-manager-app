...
router.post("/user", async (req, res) => {
  console.log("it is from req body ", req.body);
...
router.get("/user/:id", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .send({ err: "data not found", message: "try other Id" });
    }
    res.status(200).send(user);
  } catch (error) {
...
const uploadAvatar = multer({
  ...
  fileFilter(req, file, thecallback) {
    //functions with 3 parameters
    if (!file.originalname.match(/\.(jpeg|png|jpg|gif)$/)) {
      //check regex in regex101.com
